...
class Score (object):
	...
	def append_change (self, change):
		...
		if change.score is not None and (self._last is None or self._submissions [s_id].time > self._last.time):
			self._last = self._submissions [s_id]

		if self._score_mode == "max":
			score = max ([0.0] + [submission.score for submission in self._submissions.values ()])
		...
		#    score = max([0.0] + [submission.score + time_bonus for submission, time_bonus in zip(self._submission.values(), time_bonuses)])
			...
			if self._submissions:
				...
				# assert isinstance (task, Task), ("not a task!!" + str (type (task)))
	...
	def create_subchange (self, key, subchange):
		...
		# or reset_history)
		if len (self._changes) == 0 or subchange.time > self._changes [-1].time or (
			...
			for idx, val in enumerate (self._changes):
				if subchange.time < val.time or (subchange.time == val.time and subchange.key < val.key):
					self._changes.insert (idx, subchange)
					break
			self.reset_history ()
			logger.info ("Reset history for user '%s' and task '%s' after "
			             "creating subchange '%s' for submission '%s'", self._submissions [subchange.submission].user,
			             self._submissions [subchange.submission].task, key, subchange.submission)
...
class ScoringStore (object):
	...
	def create_submission (self, key, submission):
		...
		if submission.task not in self._scores [submission.user]:
			...
			self._scores [submission.user] [submission.task] = Score (score_mode = task ["score_mode"])
	...
	def update_submission (self, key, old_submission, submission):
		if old_submission.user != submission.user or old_submission.task != submission.task:
	...
	def delete_submission (self, key, submission):
		...
		if len (self._scores [submission.user] [submission.task]._submissions) == 0:
